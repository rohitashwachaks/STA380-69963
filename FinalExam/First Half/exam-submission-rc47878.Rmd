...
colnames(mat) <- c("Coefficient","P-Value","R^2")
...
plot(x = mat[,1], xlab = "Simple Linear Regression Coefficients",
     y = smry_mul$coefficients[-1,1], ylab = "Multiple Linear Regression Coefficients",
     main = "Parameter Coefficents Simple v/s Multiple"
)

cat("Coefficients of Simple Linear Regression\n")
mat


cat("\n\nCoefficients of Multiple Linear Regression\n")
smry_mul$coefficients
```
From the plot and the tables, we see that most of the predictors are around the same range of values, except **nox**.

- $\beta_{nox}$ went from 31.25 in the case of Simple Linear Regression to -10.31 in Multiple Linear Regression.
- Similarly, $\beta_{zn}$ also flipped signs and went from -0.07 to 0.04
...
Thus, from the P-Value, we can conclude that, for a 95% confidence interval, there exists a statistically significant non-linear relationship between crime rate and the following predictors:
...
plot(test$Apps, xlab= "Actual Admits", 
     test$Apps - ridge_preds, ylab = "Prediction Error",
     main = "Residuals in Ridge Regression")
abline(1,1, col="red")
```
***
<a id="6.9"></a>
...
As is clear from the model outputs, KNN did a very poor job at classifying Heart Strokes
from just the dataset we had. However, It paved the way for more complex models and we were able to work on Bagging, Boosting and Random Forest models which proved to be far more effective in dealing with the categortical predictors.
